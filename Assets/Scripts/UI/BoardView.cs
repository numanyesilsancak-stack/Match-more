...
namespace Game.Board
{
    public sealed class BoardView : MonoBehaviour
    {
        ...
        {
            x = y = -1;
            if (!RectTransformUtility.ScreenPointToLocalPointInRectangle(boardRoot, screenPos, uiCam, out var local)) return false;

            float half = _boardSize * 0.5f;
            float lx = local.x + half;
            float ly = local.y + half;

            if (lx < 0f || ly < 0f || lx >= _boardSize || ly >= _boardSize) return false;
        ...
        {
            return ui.Rt.DOAnchorPos(CellToLocal(x, y), duration).SetEase(Ease.OutQuad);
        }
        ...
        public Tween TweenSpawnFromAbove(TileUI ui, int x, int y, float duration, float extraHeight = 2.5f)
        {
            ...
            return ui.Rt.DOAnchorPos(target, duration).SetEase(Ease.OutBounce);
        }
